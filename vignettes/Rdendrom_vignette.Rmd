...
It is important to **NOTE** that the output of the fitting functions are **files** and not objects. The default folder is the present working directory, but I recommend making an "OUTPUT" directory to hold the sequenc of these files. The output files from the first fitting function are input to the second. The output from the second *ammend the first* but do not overwrite them. I'd be glad to adjust this as folks see fit.
...
There are four files from the output. Three are list objects that contain the same information, but are in different structures, and the fourth is a csv file, param.table, of parameters and extra metrics. The list files are all of the time series data either
  - combined,  *Dendro.complete*
  - split by tree id, *Dendro.tree*
  - split by tree id, year, and band (i.e., the minimal designation of a time series of diameters), *Dendro.split*.
...
get.extra.metrics(
  ...
  Dendro.split,
  OUTPUT.folder      = "OUTPUT",
  param.table.name = sprintf("Param_table_complete_%s.csv", INPUT.dendro$DATA_SET[1]),
  Dendro.data.name = sprintf("Dendro_data_complete_%s.Rdata", INPUT.dendro$DATA_SET[1]),
  Dendro.split.name = sprintf("Dendro_data_split_complete_%s.Rdata", INPUT.dendro$DATA_SET[1]),
...
################################################
### QUICK TEST OF R-SQUARES AND FITS         ###
###  WITH OPTIONS TO PLOT AND FIND OUTLIERS. ###
################################################
param.table.name = sprintf("Param_table_complete_%s.csv", INPUT.dendro$DATA_SET[1])
Dendro.data.name = sprintf("Dendro_data_complete_%s.Rdata", INPUT.dendro$DATA_SET[1])
Dendro.split.name = sprintf("Dendro_data_split_complete_%s.Rdata", INPUT.dendro$DATA_SET[1])
...
summary(subset(param.table, r.squared > 0.75))
boxplot(param.table$r.squared ~ param.table$SP)
boxplot(param.table$ts.sd ~ param.table$SP)
# r2 <- param.table$r.squared

# plot(r2, log(param.table$ts.sd))
# plot(hist(r2, 50))
# low.r2 <- which(param.table$r.squared < 0.75 & !is.na(param.table$r) &
#    param.table$flag == 0)
# param.low.r2 <- param.table[low.r2, ]
# Dendro.data.low.r2 <- Dendro.split[low.r2]

########################
source('CODE/DENDRO_PLOT_FUNCTIONS.R')
ts.data <- Dendro.split[[3]]
params <- param.table[3, ]
``` -->
...
# Make a pdf of selected datasets
plot.dendro(ts.data, params = params, r.square = NULL,
  main.title = "", day = seq(365))

  # This takes multiple years (and bands) from the same tree
plot.dendro.multiple(Dendro.data.ls, par.dim = c(3, 2), print.pdf = FALSE, out.file = "FIGURES/TEST_PLOTS.pdf",
  param.tab = param.low.r2, day = seq(365))
...
# find outliers to re-insert into optimization.
which.outlier <- find.outliers(ts.data, sd.lim = 3)

new.outlier.df <- id.outliers(Dendro.data.low.r2,
  param.tab = param.low.r2)

INPUT.dendro.2 <- integrate.outlier.id(Dendro.complete, new.outlier.df)
